...
int main(void) {
    ...
    block = cuda::max_blocksize(cuda::agree_subpixel_kernel<INPUT_TYPE, double, cuda::nxcorrd<true>>);
    grid = create_grid(block, randsize);

    cuda::agree_subpixel_kernel<INPUT_TYPE, double, cuda::nxcorrd<true>>
        <<<grid, block>>>(randdisp_dev, devptr, n, thresh, step, minvar, devout_gmem);
    assertCudaSuccess(cudaGetLastError());

    assertCudaSuccess(cudaFuncSetAttribute(
        impl::cuda::agree_subpixel_kernel_smem<INPUT_TYPE, double, cuda::nxcorrd<true>>,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        smem_size
    ));

    block = cuda::max_blocksize(cuda::agree_subpixel_kernel_smem<INPUT_TYPE, double, cuda::nxcorrd<true>>, smem_size);
    grid = create_grid(block, randsize);

    cuda::agree_subpixel_kernel_smem<INPUT_TYPE, double, cuda::nxcorrd<true>>
